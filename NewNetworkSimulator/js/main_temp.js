...
NSF.conev = {
  ...
  hide: function(opt) {
    ...
    //コンソールに情報を表示
    if (this.data().ip !== "" && this.data().sm !== "") {
      $("#nsf-console").append("<p>" + opt.$trigger[0].alt + "のIPを " + this.data().ip + "/" + this.data().sm + " に設定しました" + "</p>");
    }
  }
}
...
NSF.conev_2 = {
  ...
  hide: function(opt) {
    ...
    $("#nsf-right dt:contains('"+ opt.$trigger[0].alt +"') + dd p:nth-of-type(3) span:first").text(this.data().next_hop_add);
    console.log(this.data().next_hop_add);
    //コンソールに情報を表示
    $("#nsf-console").append("<p>" + opt.$trigger[0].alt + "のIP-1を " + this.data().ip_1 + "/" + this.data().sm_1 + ", IP-2を "+ this.data().ip_2 + "/" + this.data().sm_2 + " に設定しました" + "</p>");
  }
}
...
NSF.fnstudySelect = function () {
  ...
  $.ajax({
    ...
    dataType: 'text',
    url: 'php/DB.php',
    data:{question_id : studyTitleValue}
  }).done(function(data) {
    $("#nsf-console").append("<p>> 問題を取得しました </p>");
    question.txt.value = data;
  }).fail(function(XMLHttpRequest, textStatus) {
...
NSF.fnMainDrop = function(ui, obj) {
  ...
  if(ui.draggable.attr("src") === "img/pc.png") {
    ...
    $("#nsf-right dl").append("<dt><img src= img/plus.jpg>"+ ui.draggable.attr("alt") + + NSF.dropNodeInt +"</dt><dd><p>IP-1: <span></span> /<span>24</span></p><p>IP-2: <span></span> /<span>24</span></p><p>NHA: <span></span></p></dd>");
  }
  // dd要素(IPとSM)を隠す
...
$("#connect-start").click(function(){
  ...
  // Get-NodeかSend-Nodeを付いているかの確認とNSFCS.totalNodeNumを決定する。(lanLinkがついているもの総数)
  $("#nsf-main .lanLink").each(function(e){
  ...
  $.ajax({
    type: 'POST',
    dataType: 'text',
    url: 'php/post_networkdata.php',
    data:{postData : NSFCS.postData}
  }).done(function(data) {
    $("#nsf-console").append("<p>>result･･･" + data +  "</p>");
  ...
  NSF.fnGlayOpen();
    $("#glayLayer").append('<img src="img/batu.png" id="glayClose">'+
      ...
      '<div id="studyMenuOutput">'+
        ...
        '<input id="studySelectTitle" placeholder="タイトルを入力して下さい">'+
        '<button id="studySelect">問題を選択</button>'+
        '</div>'+
      '</div>'
    );
